...
public class PostForm extends SinglePhaseAction {
  ...
  protected void performSinglePhase() {
    ...
    try {
      StringBuilder scriptStr = new StringBuilder(
          String.format("var formE = document.createElement('form');formE.setAttribute('id','%s');"
      ...
      if (this.acceptCharset != null && !this.acceptCharset.isEmpty()) {
        scriptStr.append("formE.setAttribute('accept-charset','"
      ...
      if (this.formData != null) {
        ...
        if (!formDataList.isEmpty()) {
          ...
          for (SimpleImmutableEntry<String, String> input:formDataList) {
            scriptStr.append(
                String.format("customE = document.createElement('input');"
      ...
      scriptStr.append(
          String.format("document.body.appendChild(formE);document.getElementById('%s').submit();",
              formId.replaceAll("'", "\'")));
      try {
        ((JavascriptExecutor)actionComposer.getWebDriver()).executeScript(scriptStr.toString());
      } catch (Exception ex) {
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug("{}({}): Execute javascript error:{}", ActionComposer.class.getSimpleName(),
              actionComposer.getName(), toString(), ex);
        }
      }
    } catch (Exception e) {
  ...
  public String toString() {
    return String.format("%s:%s/%s/%s", PostForm.class.getSimpleName(), url,
        String.join(",", formData.stream().map(s -> s.getKey() + "=" + s.getValue())
            .collect(Collectors.toList())), acceptCharset);
  }
}