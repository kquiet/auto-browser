 ...
 * executed and track their execution result. If any executed action fails, {@link ActionComposer}
...
public interface ActionComposer extends Runnable, Prioritized, DynamicActionSequence {
   ...
   *
   * @return all registered windows represented by {@link Map} (register name paire with window
   *     identity)
   */
  ...
  /**
   * Delegate the execution of {@link Runnable} to associated {@link ActionRunner} with this {@link
   * ActionComposer}'s priority.
   *
   ...
   *
   * @return {@code true} if this {@link ActionComposer} has been marked as failed; {@code false}
   *     otherwise
   */
  boolean isFail();
  ...
  /**
   * Get the url of the last page when this {@link ActionComposer} is marked as failed and {@link
   * #keepFailInfo(boolean) the flag to keep fail information} is enabled.
   *
   ...
   * marked as failed. The function of keeping fail information takes about one second to complete,
   * however this may seem wasteful in many applications, hence this method can be used to determine
   * keep or not.
   *
  ...
  /**
   * Set the function to be executed when this {@link ActionComposer} is finished without being
   * marked as failed.
   *
  ...
  /**
   * Set the function to be executed when this {@link ActionComposer} is done. This function is
   * executed after <i>fail function</i> and <i>success function</i>.
   *
  ...
  /**
   * Get {@link WebDriver} from associated {@link ActionRunner}. Use this with caution because the
   * associated {@link ActionRunner} use the same {@link WebDriver} when executing browser actions,
   * however {@link WebDriver} is <a
   * href="https://github.com/SeleniumHQ/selenium/wiki/Frequently-Asked-Questions#q-is-webdriver-thread-safe"
   * target="_blank">not thread-safe</a>.
   *
   * <p>A safer way to use this is to encapsulate the process in a {@link Runnable}, or use built-in
   * {@link org.kquiet.browser.action.Custom custom action}, and then execute it through {@link
   * ActionRunner#executeAction(java.lang.Runnable, int)}.
   *
